...
import static org.meanbean.test.SignificantObjectPropertyEqualityConsistentAsserter.formatVariableString;
...
class InsignificantObjectPropertyEqualityConsistentAsserter implements ObjectPropertyEqualityConsistentAsserter {
   ...
   * @param originalObject        The object that the modified object was logically equivalent to prior to its modification.
   * @param modifiedObject        The modified object, which was logically equivalent to the original object prior to the modification
   *                              of its named property.
   * @param originalPropertyValue The original value of the modified property. This should still be the value of the named property in
   *                              the original object.
   * @param newPropertyValue      The new value of the named property in the modified object.
   *
   * @throws IllegalArgumentException If any of the parameters are deemed illegal. For example, if any are <code>null</code>.
   * @throws AssertionError           If the equality of the modified object is inconsistent with the changes made to its property.
   */
  @Override
  public void assertConsistent(
      String propertyName, Object originalObject, Object modifiedObject,
    ...
    if (!originalObjectEqualsModifiedObject && !newPropertyValueEqualsOriginalPropertyValue) {
      String message = "objects that differ due to supposedly insignificant property [" + propertyName
          + "] where considered unequal. " + variableString.get() + ". is property [" + propertyName
          + "] actually significant?";
      AssertionUtils.fail(message);

    } else if (!originalObjectEqualsModifiedObject && newPropertyValueEqualsOriginalPropertyValue) {
      String message = "objects that should be equal were considered unequal when testing insignificant " + "property ["
