...
public final class PropertyInformationFilter {
  ...
  public enum PropertyVisibility implements Predicate<PropertyInformation> {
    ...
    READABLE() {
      ...
      boolean isRelevant(PropertyInformation propertyInformation) {
        return propertyInformation != null && propertyInformation.isReadable();
      }
    },
    ...
    WRITABLE() {
      @Override
      boolean isRelevant(PropertyInformation propertyInformation) {
        return propertyInformation != null && propertyInformation.isWritable();
      }
    },
    ...
    READABLE_WRITABLE() {
      @Override
      boolean isRelevant(PropertyInformation propertyInformation) {
        return propertyInformation != null && propertyInformation.isReadableWritable();
      }
    };
     ...
     *
     * @param propertyInformation The property that will either be ignored or included.
     *
   ...
   *
   * @param properties A Collection of PropertyInformation objects to be filtered.
   * @param filter     The filter to be used. This filter will specify whether properties that are readable, writable or both
   *                   are relevant.
   *
   * @return A Collection of PropertyInformation objects deemed relevant by the specified filter.
   *
   * @throws IllegalArgumentException If either of the required parameters are deemed illegal. For example, if either is null.
   */
  public static List<PropertyInformation> filter(
